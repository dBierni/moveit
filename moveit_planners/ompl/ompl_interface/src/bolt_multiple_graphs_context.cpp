...
void ompl_interface::BoltTaskGraphGenerator::operator()(const Eigen::Isometry3d &current_pose, const ModelBasedStateSpacePtr state_space)
{
  ...
  std::vector<double> offset;
   graph_full_->first.translation().z() = 0.35; // TODO  ompl-rosparam not loaded ?
    ...
    BOOST_FOREACH(const otb::SparseEdge edge, boost::edges(graph))
          {
            ...
            std::vector<double> v1_vec, v2_vec;
            if(!diff)
            {
              offset = getOffsetForState(graph[v1].state_, joint_model_group, state_space, current_pose);
              v1_vec.resize(offset.size());
              v2_vec.resize(offset.size());
              ...
              std::transform (v1_vec.begin(), v1_vec.end(), offset.begin(), v1_vec.begin(), std::minus<double>());
              std::transform (v2_vec.begin(), v2_vec.end(), offset.begin(), v2_vec.begin(), std::minus<double>());
...
Eigen::Isometry3d ompl_interface::BoltTaskGraphGenerator::getSearchPose(const Eigen::Isometry3d &pose)
{
  ...
  Eigen::Isometry3d result = Eigen::Isometry3d( Eigen::Translation3d(
          (pose.translation().x()-graph_root.translation().x()),
                                                                      ...
                                                                      const Eigen::Isometry3d & current_pose)
{
  ...
  ROS_INFO_STREAM("Pose1:  x:" << pose.translation().x() <<"| y: " <<pose.translation().y() << "| z: "<<pose.translation().z());
  ...
  ROS_INFO_STREAM("Pose:  x:" << pose.translation().x() <<"| y: " <<pose.translation().y() << "| z: "<<pose.translation().z());

  ompl::base::State *to = poseToModelSpaceState(pose, joint_model_group, state_space->allocState());
